...
# NullPool test engine to avoid event-loop connection pooling conflicts
...
async def clean_database_and_redis():
    ...
    async with test_engine.begin() as conn:
        await conn.execute(text("TRUNCATE TABLE social_accounts CASCADE;"))

    keys = (
        redis_client.keys("token:*")
        + redis_client.keys("jobs:social-account*")
    )
    if keys:
