...
| **State Progression Monotonicity**| Job execution steps MUST move strictly forward in sequence (`started` -> `db_stored` -> `published_event` -> `completed`). | `StateManager` relational table `job_execution_state` and Redis step updates. | Step resume logic checks `get_last_step()` and bypasses completed milestones. |
