...
def inspect_dlq(service_name: str, count: int = 10):
    ...
    try:
        messages = redis_client.xrange(stream_name, count=count) or []
...
def replay_dlq_job(service_name: str, message_id: str):
    ...
    try:
        ...
        new_id = redis_client.xadd(
            target_stream, {"payload": json.dumps(payload), "status": "pending"}
        )
        redis_client.xdel(dlq_stream, message_id)
        return {
            "status": "requeued",
            "new_job_id": new_id.decode("utf-8")
