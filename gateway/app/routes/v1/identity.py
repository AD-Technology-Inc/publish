...
async def get_users():
    ...
    return await _forward(ServiceName.IDENTITY, "GET", "http://identity-service:3001/users")


@identity_router.get("/{user_id}")
async def get_user(user_id: str):
    from main import ServiceName
    return await _forward(ServiceName.IDENTITY, "GET", f"http://identity-service:3001/users/{user_id}")
...
async def create_user(request: Request):
    ...
    # [x] 2. CIRCUIT BREAKER: Trip the connection if identity-service fails repeatedly to prevent thread starvation.
    ...
    # [x] 5. RETRIES: Add safe GET retries (using exponential backoff) for transient upstream connection drops.
    return await _forward(ServiceName.IDENTITY, "POST", "http://identity-service:3001/users", request)